...
static LPCWSTR get_profile_string(LPCWSTR name, LPCWSTR key, LPCWSTR def, LPCWSTR filename)
{
	wchar_t temp[1024];
	int result = GetPrivateProfileString(name, key, def, temp, sizeof(temp), filename);
	return (LPCWSTR)std::wstring(temp, result).c_str();
}
...
bool logload = GetPrivateProfileInt(L"core", L"logload", 1, ini);

LPCWSTR logname = get_profile_string(L"core",L"logname",L"debug.txt",ini);
